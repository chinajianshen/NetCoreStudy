...
namespace Transfer8Pro.Client.Controls
{
    public partial class TaskPlanDayCtrl : UserControl, ITaskPlanBase
    {
        ...
        {
            rdoOneTimes.CheckedChanged -= rdoOneTimes_CheckedChanged;
            rdoMultiTimes.CheckedChanged -= rdoMultiTimes_CheckedChanged;
            cbxOneTimesHour.SelectedIndexChanged -= cbxOneTimesHour_SelectedIndexChanged;
            cbxOneTimesMinute.SelectedIndexChanged -= cbxOneTimesMinute_SelectedIndexChanged;
            cbxMultiTimesMinute.SelectedIndexChanged -= cbxMultiTimesMinute_SelectedIndexChanged;
            ...
            if (cronExp != null)
            {
                ...
                else
                {
                    if (cronExp.Minute != "0")
                    {
                        ...
                        string text = $"{cronExp.Minute.TrimStart(new char[] { '0', '/' })}分";
                        int index = Common.DigitalMultiTimesMinutes().FindIndex(item => item.K == text);
                        cbxMultiTimesMinute.SelectedIndex = index > 0 ? index : 1;

                    }
                    else if (cronExp.Hour != "*" && cronExp.Hour != "0")
                    {
                        //cbxMultiTimesMinute.SelectedValue = cronExp.Hour;
                        string text = $"{cronExp.Hour.TrimStart(new char[] { '0', '/' })}小时";   
                        int index = Common.DigitalMultiTimesMinutes().FindIndex(item => item.K == text);
                        cbxMultiTimesMinute.SelectedIndex = index > 0 ? index : 1;
                    }
