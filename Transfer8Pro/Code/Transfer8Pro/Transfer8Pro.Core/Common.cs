...
namespace Transfer8Pro.Core
{
    public class Common
    {
        ...
        public static string DecryptData(string decryptString)
        {
            ...
            {
                throw new ArgumentNullException("DecryptData()方法，参数decryptString值为空");
            }         
        ...
        public static string EncryptData(string encryptString)
        {
            ...
            {
                throw new ArgumentNullException("EncryptData()方法，参数encryptString值为空");
            }
        ...
        public static string DecryptData(string decryptString,string encryptKey)
        {
            if (string.IsNullOrEmpty(decryptString))
            {
                throw new ArgumentNullException("DecryptData()方法，参数decryptString值为空");
            ...
            {
                throw new ArgumentNullException("DecryptData()方法，参数encryptKey值为空");
            }          

            return RijndaelCrypt.Decrypt(decryptString, encryptKey);
        }
        ...
        /// <returns></returns>
        public static string EncryptData(string encryptString, string encryptKey)
        {
            if (string.IsNullOrEmpty(encryptString))
            {
                throw new ArgumentNullException("EncryptData()方法，参数encryptString值为空");
            ...
            if (string.IsNullOrEmpty(encryptKey))
            {
                throw new ArgumentNullException("EncryptData()方法，参数encryptKey值为空");
            }


            return RijndaelCrypt.Encrypt(encryptString, encryptKey);
        }
        ...
        /// <returns></returns>
        public static string DecryptConfigKey(string configKey)
        {
            string key = ConfigHelper.GetConfig(configKey);
            return RijndaelCrypt.Decrypt(key, RijndaelCrypt.EncryptKey);
        }
        ...
        public static ADbFileProductBuilder GetDbFileProductBuilder()
        {
            ...
            int systype = DecryptConfigKey("OpenBookSystemType").ToInt();
        ...
        public static bool IsExistSQLiteDB()
        {
            ...
            {
                var sysconfigs = new SystemConfigService().GetSystemConfigList();
                if (sysconfigs == null || sysconfigs.Count == 0)
                {
                    return false;
                }
            }
        ...
        /// <returns></returns>
        public static KVEntity<string> DbReservedString(DbTypes dbType)
        {
            KVEntity<string> entity = new KVEntity<string>(); ;
            switch (dbType)
            {
...
WHERE SalesDateTime>= @StartTime and SalesDateTime<= @EndTime";
                    entity.T1 = @"SELECT isbn,Title as  booktitle,PublishName as publisher_name,price, SalesCount as amount,Author as author_editor,SalesDateTime as selldate,StoreID as werks
FROM T8_BookInfo";
                    break;
                case DbTypes.Oracle:
...
WHERE publish1id>0 and WriteTime>= :StartTime and WriteTime <= :EndTime";
                    entity.T1 = @"SELECT isbn,Title booktitle,price,PublishName publisher_name,WriteTime selldate 
FROM ob_bookinfo_m ";
                    break;
                case DbTypes.MySql:
                    entity.K = "server=192.168.3.249;database=test;userid=root;password=123456;port=3306;Charset=utf8;Allow User Variables=True;";
                    entity.V = @"SELECT id,servername,dbname,objectname,objecttype,objectdefine,create_date 
FROM test 
WHERE create_date>=@StartTime AND create_date<=@EndTime";
                    entity.T1 = @"SELECT id,servername,dbname,objectname,objecttype,objectdefine,create_date 
FROM test ";
                    break;
                case DbTypes.Oledb:
        ...
        /// </summary>
        /// <returns></returns>
        public static List<KVEntity> GetFtpUplpadStatusList()
        {
            List<KVEntity> list = new List<KVEntity>();
            list.Add(new KVEntity { K = "", V = "-999" });
            foreach (FtpUploadStatus dataType in Enum.GetValues(typeof(FtpUploadStatus)))
            {
                list.Add(new KVEntity { K = dataType.ToString(), V = ((int)dataType).ToString() });
            }
            return list;
        ...
        /// </summary>
        /// <returns></returns>
        public static List<KVEntity> GetPosTypeList()
        {
            List<KVEntity> list = new List<KVEntity>();
            list.Add(new KVEntity { K = "", V = "-999" });
            foreach (PosTypes dataType in Enum.GetValues(typeof(PosTypes)))
            {
                list.Add(new KVEntity { K = dataType.ToString(), V = ((int)dataType).ToString() });
            }
            return list;
        ...
        /// </summary>
        /// <returns></returns>
        public static List<KVEntity> GetTaskExecutedStatusList()
        {
            List<KVEntity> list = new List<KVEntity>();
            list.Add(new KVEntity { K = "", V = "-999" });
            foreach (TaskExecutedStatus dataType in Enum.GetValues(typeof(TaskExecutedStatus)))
            {
                list.Add(new KVEntity { K = dataType.ToString(), V = ((int)dataType).ToString() });
            }
            return list;
