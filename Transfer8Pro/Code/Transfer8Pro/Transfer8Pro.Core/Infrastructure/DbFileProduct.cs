        ...
        public override void BuildDbFile()
        {
            Action<TaskEntity> action = t8Task =>
            {
                try
                {
                    ...
                    //1得到数据库文件路径
                    AFileName fileNameStragety = buildInstanceService.GetGenerateFileNameStragety(1,t8Task);
                    ...
                    taskLog.TaskID = t8Task.TaskID;
                    taskLog.DataType = t8Task.DataType;
                    taskLog.CycleType = t8Task.CycleType;
                    taskLog.FileName = fileInfoEntity.NormalFileName;
                    int taskLogDetailID = 0;
                    if (taskLogService.InsertOrUpdateLog(taskLog, out taskLogDetailID))
                    {
                        t8Task.TaskLogDetailID = taskLogDetailID;
                    }
                ...
                {
                    LogUtil.WriteLog(ex);
                    if (t8Task.TaskLogDetailID > 0)
                    {
                        taskLogService.UpdateErrorLog(t8Task.TaskID, t8Task.TaskLogDetailID, $"[{ex.Message}][{ex.StackTrace}]");
                    }
        ...
        public override void BuildCompressFile()
        {
            Action<TaskEntity> action = t8Task =>
            {
                try
                {
                    ...
                    AFileName fileNameStragety = new BuildInstanceObject().GetGenerateFileNameStragety(2,t8Task);
            ...
            Action<TaskEntity> action = t8Task =>
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        //添加上传日志
                        FtpUploadLogEntity ftpUploadLog = new FtpUploadLogEntity();
                        ftpUploadLog.CycleType = t8Task.CycleType;
                        ftpUploadLog.DataType = t8Task.DataType;
                        ftpUploadLog.FileName = FileHelper.GetFileName(ftpFilePath);
                        ftpUploadLog.CreateTime = DateTime.Now;
                        ftpUploadLog.FtpUploadStatus = FtpUploadStatus.等待上传;
                        new FtpUploadLogService().Insert(ftpUploadLog);

                        //添加任务日志
                        if (t8Task.TaskLogDetailID > 0)
                        {
                            taskLogService.UpdateSuccessLog(t8Task.TaskID, t8Task.TaskLogDetailID, FileHelper.GetFileName(ftpFilePath));
                        }
        ...
        public override void BuildDbFile()
        {
            ...
            {               
                try
                {
                    BuildInstanceObject buildInstanceService = new BuildInstanceObject();
                    //1得到数据库文件路径
                    AFileName fileNameStragety = buildInstanceService.GetGenerateFileNameStragety(3,t8Task);
                    ISqlQueryTime SqlQueryTimeStragety = buildInstanceService.GetSqlQueryTimeStragety(t8Task.CycleType);
                    ...
                    //传8周月（销售 在架）不加密
                    if (RegexExpressionUtil.T8DataFileFormatReg.IsMatch(fileInfoEntity.NormalFilePath))
                    {
                        isSuccess = handler.OutputDataToSQLite(sql, fileInfoEntity.SqlStartTime, fileInfoEntity.SqlEndTime, fileInfoEntity.NormalFilePath);
                    }
                    else
                    {
                        isSuccess = handler.OutputData(sql, fileInfoEntity.SqlStartTime, fileInfoEntity.SqlEndTime, fileInfoEntity.NormalFilePath);
                    } 
        ...
        public override void BuildCompressFile()
        {
            Action<TaskEntity> action = t8Task =>
            {
                try
                {
                    ...
                    AFileName fileNameStragety = new BuildInstanceObject().GetGenerateFileNameStragety(4,t8Task);
                    t8Task.FileInfo.CompressFileName = fileNameStragety.FileName(t8Task);
                    t8Task.FileInfo.CompressFilePath = fileNameStragety.FileFullName(t8Task);

                    //传8周月（销售 在架）不加密
                    if (RegexExpressionUtil.T8DataFileFormatReg.IsMatch(t8Task.FileInfo.CompressFilePath))
                    {
                        FileHelper.ZipFile(t8Task.FileInfo.NormalFilePath, t8Task.FileInfo.CompressFilePath, "");
                    }
                    else
                    {
                        FileHelper.ZipFile(t8Task.FileInfo.NormalFilePath, t8Task.FileInfo.CompressFilePath, Common.GetEncryptKey());
                    }
                    ...
                    {
                        //删除一般文件出错不影响整体，这里进行容错处理
                        File.Delete(t8Task.FileInfo.NormalFilePath);
                    }
                    catch (Exception ex)
                    {
                        LogUtil.WriteLog($"压缩数据操作完成，清理没有用处一般数据文件[{t8Task.FileInfo.NormalFilePath}]时出错,异常信息：[{ex.Message}]");
                    }
