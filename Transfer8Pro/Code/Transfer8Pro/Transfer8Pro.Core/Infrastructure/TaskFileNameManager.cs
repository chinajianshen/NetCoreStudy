...
namespace Transfer8Pro.Core.Infrastructure
{
    ...
    {   
        ...
        /// <summary>
        /// 一般文件名格式
        /// 天数据： 20181012_093424_d_book.jl     
        /// 周数据： 20190318_20190324_w_sale_87jdih_ojdh73.jl
        /// 月数据： 20190301_20190331_m_sale_4ijd89_2hi84n.jl
        /// </summary>
        /// <param name="taskEntity"></param>
        /// <returns></returns>
        public override string FileName(TaskEntity taskEntity)
        {
            if (string.IsNullOrEmpty(fileName))
            {
                ...
                fileName = string.Format("{0}_{1}_{2}_{3}.jl"
                    , taskEntity.CycleType == CycleTypes.D ? DateTime.Now.ToString("yyyyMMdd"): sqlQueryTime.StartTime(DateTime.Now).ToString("yyyyMMdd")
                    , taskEntity.CycleType == CycleTypes.D ? DateTime.Now.ToString("HHmmss") :sqlQueryTime.EndTime(DateTime.Now).ToString("yyyyMMdd")
    ...
    {       
        private Regex normalFileValidateReg = new Regex(@"^\d{8}_\d{6,8}_\w{1}_\w{4,10}.jl$", RegexOptions.Singleline | RegexOptions.IgnoreCase | RegexOptions.Compiled);
        public override string FileFullName(TaskEntity taskEntity)
        {
    ...
    {    
        ...
        public override string FileName(TaskEntity taskEntity)
        {
            ...
            fileName = FileHelper.GetFileName(taskEntity.FileInfo.NormalFilePath) +".zip";
            return fileName;
        }
    }
    #endregion
