    ...
    public class AutoUploadFtpJobService : JobService<AutoUploadFtpJob>
    {
        ...
        protected override ITrigger GetTrigger()
        {
            try
            {
                ...
                string cronExp = "0 0/1 * * * ? *"; //自动上传作业服务没有配置，则默认1分钟执行一次
                SystemConfigEntity systemConfig = new SystemConfigService().FindSystemConfig((int)SystemConfigs.FtpUpoladService);
