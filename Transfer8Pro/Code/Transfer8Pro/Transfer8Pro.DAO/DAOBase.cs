...
namespace Transfer8Pro.DAO
{
    ...
    public class DAOBase<D> where D : DbConnection, new()
    {
        ...
        protected T ExecuteFor<T>(Func<IDbConnection, T> act,bool isThrowError=false, string connect_string = null)
        {
            try
            {
        ...
        protected void ExecuteFor(Action<IDbConnection> act, bool isThrowError =false, string connect_string = null)
        {
            try
            {
                using (IDbConnection con = new D() { ConnectionString = connect_string ?? _default_connect_str })
        ...
        protected bool ExecuteForWithTrans(Func<IDbTransaction, bool> act, bool isThrowError =false, string connect_string = null)
        {
        ...
        protected T ExecuteForWithTrans<T>(Func<IDbTransaction, T> act, Func<T, bool> handleRtFunc, bool isThrowError = false, string connect_string = null)
        {
            IDbTransaction trans = null;
            try
            {
        ...
        protected virtual string GetSqlServerConnectString()
        {
#if DEBUG
            return ConfigHelper.GetDBConnectStringConfig("OB_Trans8_DB_ConStr_Dev");
#else
             return ConfigHelper.GetDBConnectStringConfig("OB_Trans8_DB_ConStr");
#endif
        }
    }
    ...
    public class DataHandler_DAOBase<D> : DAOBase<D>, IDataHandler where D : DbConnection, new()
    {
        ...
        public bool OutputData(string sql, DateTime op, DateTime ed, string target_jl_file, string connect_string = null)
        {
            ExecuteFor(c =>
            {
                ...
                {
                    Thread.Sleep(500);
                    File.Delete(target_jl_file);
                    throw new Exception("本次查询总条数为0");
                }
            },true, connect_string);
            return true;
        }
        ...
        public bool OutputDataToSQLite(string sql, DateTime op, DateTime ed, string sqliteFileFullpath, string connect_string = null)
        {
            ...
            ExecuteFor(c =>
            {
                ...
                {
                    targetconn.Open();
                    using (SQLiteTransaction targetTrans = targetconn.BeginTransaction())
                    {
                        try
                        {
                            using (SQLiteCommand targetcmd = new SQLiteCommand(insertSql, targetconn, targetTrans))
                            {
                        ...
                        {
                            //LogUtil.WriteLog($"OutputDataToSQLite()方法异常，异常信息[{ex.Message}][{ex.StackTrace}]");
                            if (targetTrans != null)
                            {
                                targetTrans.Rollback();
                            }
                            throw new Exception(ex.Message);
                        }
                    }
                ...
                {
                    Thread.Sleep(500);
                    File.Delete(sqliteFileFullpath);
                    throw new Exception("本次查询总条数为0");
                }
