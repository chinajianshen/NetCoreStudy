    ...
    public class TaskDAO : DAOBase<SQLiteConnection>
    {
        ...
        public List<TaskEntity> GetAllTaskList()
        {
            var sql = @"SELECT * FROM T8_Task WHERE IsDelete=0  Order By CreateTime desc;
                        SELECT * FROM T8_FtpConfig where ftptype=1;";
        ...
        /// <returns></returns>
        public List<TaskEntity> GetTaskList(List<string> taskIdList)
        {
            if (taskIdList == null || taskIdList.Count == 0)
                return new List<TaskEntity>();

            string taskIDs = string.Join(",",taskIdList.Select(item => $"'{item}'"));
            string sql = $"Select * From T8_Task Where IsDelete=0 AND TaskID In ({taskIDs}); SELECT * FROM T8_FtpConfig where ftptype=1;";
            return base.ExecuteFor(conn =>
            {
                var multiResult = conn.QueryMultiple(sql);
                List<TaskEntity> list = multiResult.Read<TaskEntity>().ToList();
        ...
        {
            var sql = @"INSERT INTO T8_Task (TaskID,TaskName,Cron,DataHandler,DBConnectString_Hashed,SQL,DataType,IsDelete,TaskStatus,CreateTime,ModifyTime,RecentRunTime,NextFireTime,Remark,Enabled,CycleType,PosTypes) 
                       VALUES(@TaskID,@TaskName,@Cron,@DataHandler,@DBConnectString_Hashed,@SQL,@DataType,@IsDelete,@TaskStatus,@CreateTime,@ModifyTime,@RecentRunTime,@NextFireTime,@Remark,@Enabled,@CycleType,@PosTypes)";
        ...
        public bool Update(TaskEntity taskEntity)
        {
            var sql = @"UPDATE T8_Task SET TaskName=@TaskName,Cron=@Cron,DataHandler=@DataHandler,DBConnectString_Hashed=@DBConnectString_Hashed,SQL=@SQL,DataType=@DataType,IsDelete=@IsDelete,TaskStatus=@TaskStatus,
                               CreateTime=@CreateTime,ModifyTime=@ModifyTime,RecentRunTime=@RecentRunTime,NextFireTime=@NextFireTime,Remark=@Remark,CycleType=@CycleType,Enabled=@Enabled,PosTypes=@PosTypes,UploadStatus=0 WHERE TaskID=@TaskID";
            return base.ExecuteFor((conn) =>
            {
                var prms = new
                {
        ...
        {
            var sql = @"UPDATE T8_Task 
                        SET RecentRunTime=@RecentRunTime,UploadStatus=0
                        WHERE  TaskID=@TaskID";
        ...
        {
            var sql = @" UPDATE T8_Task
                         SET NextFireTime = @NextFireTime,ModifyTime = @ModifyTime,UploadStatus=0
                         WHERE TaskID=@TaskID";
        ...
        {
            var sql = @" UPDATE T8_Task
                         SET Enabled = @Enabled,UploadStatus=0                        
                         WHERE TaskID=@TaskID";
            return base.ExecuteFor(conn =>
            {
        ...
        /// </summary>
        /// <returns></returns>
        public List<TaskEntity> GetWaitUploadList()
        {
            string sql = "SELECT * FROM T8_Task WHERE UploadStatus != 1";
            return base.ExecuteFor(conn =>
            {
                return conn.Query<TaskEntity>(sql, conn).ToList();
            });
        }
        ...
        /// <returns></returns>
        public bool UpdateUploadStataus(List<TaskEntity> list)
        {
            string sql = "Update T8_Task Set UploadStatus=@UploadStatus Where TaskID=@TaskID";
